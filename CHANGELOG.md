...
- DADS wave 3 完了（19/19）— **Combobox**, **DatePicker/Calendar**, **MegaMenu**, **MenuListBox**, **TableControls**, **Carousel**, **ImageSlider**
- i18n 最初の locale 拡張（`en`）— [L4 用語](docs/L4-terminology.md) に `en` 推奨訳列、既定文言 inventory。既定文言の props override
...
- Docs を **Private 実験場 / Public ミラー**の二リポ運用に再編（二リポ運用、`npm run check:public-export`）。戦略は `（社内戦略・非公開）` に戻し、site には恒常原則のみ
- Docs site を Public ミラーの **GitHub Pages** で公開（`main` push 自動デプロイ）。URL: https://soundlabbittechnology.github.io/SLT-Design-System-public/

### Added

- G5 consumer harness — [Runbook](（社内 adoption）)、`templates/consumer-g5/`（Lighthouse CI + E2E smoke checklist）、L6 cadence / ownership
- DADS（デジタル庁デザインシステム）参照モデル追従の方針・ギャップ分析 — DADS 参照方針 / [dads-alignment-audit-2026-07-16.md](（社内監査）)
...
- L3 metrics 週次運用 — `--since` / `--base` / `--save`、`metrics/thresholds.json`、`metrics/history/`、3× override テンプレ、L6 Runbook
- `Breadcrumb`（P1）— DADS パンくずリスト。`wrap` / `scroll`、aria-current、component tokens（ wave 2 着手）
- `Tabs`（P1）— `Tabs` / `TabList` / `Tab` / `TabPanel`、矢印キー、horizontal / vertical
- `Accordion` / `Disclosure`（P1）— DADS の開閉区別。Accordion は同種連続、Disclosure は `details` による補足
- `List` / `DescriptionList` / `Blockquote` / `Chip`（P1）— 箇条書き（項番は地テキスト）、説明リスト、引用、チップタグ
- `PageNavigation` / `StepNavigation` / `ProgressIndicator` / `SearchBox`（P1）— wave 2 完了
- DADS wave 3 需要優先バッチ（P2）— `ScrollTopButton` / `UtilityLink` / `HamburgerMenuButton` / `Drawer` / `FileUpload` / `TableOfContents` / `ResourceList` / `Image`
- wave 3 続: `HorizontalMenu` / `MenuList` / `BottomNavigation` / `MobileMenu`
...
- Storybook / テーマ overlay: `button.*` 等 Tier 3 トークンが `[data-theme]` 切替に追従
...
- 全ドキュメントを P0〜P2 完了状態に同期（README / docs 索引 / Storybook MDX ）
...
- SiteHeader スロット設計 — SiteHeader 設計決定
...
- P0 DoD: guides.md, figma-props-mapping.md
- ブランドカラー方針 — coral 正式値（判断待ち）
...
- docs/L2-components/dod.md に React / Storybook 手順を追記
...
- AI readability: エージェント規則, 機械可読サマリ, エージェント規則
