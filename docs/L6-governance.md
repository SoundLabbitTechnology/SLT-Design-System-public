...
| Locale / 用語 | `ja` 正本 + [L4 用語](./L4-terminology.md) | 翻訳 ownership、glossary 同期。[ADR-005](./decisions/ADR-005-i18n-locale-architecture.md) |
| Harness | `scripts/harness/` + 本文書群 | 自動化、失敗時の修正可能性 |
| 履歴 | CHANGELOG / ADR | なぜ変わったかを残す |
...
| 週次 / release ごと | L3 operations | `npm run metrics -- --since=1.week.ago --save`、alerts / trend review、3× override backlog（[Runbook](./L6-harness-and-loops.md#l3--operations-loop)）、G5 失敗の triage（[`templates/consumer-g5/`](../templates/consumer-g5/)） |
| release / DS bump 後 | G5 consumer | 代表 consumer で Lighthouse + E2E smoke を 1 回（product 実施、DS はテンプレ維持） |
| 四半期 | L4 principles | 公開 L0（恒常原則）と DADS 同期のレビュー（下記）。事業戦略の反証は社内方針（本ミラー外） |

現在の完了事項と未完了事項は [ROADMAP](./ROADMAP.md) にだけ保持し、本書へ時点依存の進捗表を複製しません。

## 四半期ガバナンスクラスタ

技術ドキュメントの健全性レビューを **2 セッション**で構成する。**#17（L0 公開原則）と #35（DADS）は同一四半期・同一週内**に実施する。順序は L0 → DADS を推奨。
...
| 記録先 | 社内正本の監査メモ。本ミラーは [CHANGELOG](../CHANGELOG.md) / [ROADMAP](./ROADMAP.md) に要約のみ |

### 参加者

| 役割 | 責務 |
|------|------|
| **DS owner** | アジェンダ進行、記録、PR / issue 起票 |
...
### Runbook: 四半期 L0 公開原則レビュー（#17）

[公開 L0](./L0-principles.md) と [ADR-004（Laws of UX）](./decisions/ADR-004-laws-of-ux.md) アンカーの確認。事業投資・製品ロードマップの反証は社内方針側で行う。

#### アジェンダ（60 分目安）

| # | 議題 | 参照 | 時間 |
|---|------|------|------|
| 1 | 前回アクションの確認 | 直近 CHANGELOG / ROADMAP | 10m |
| 2 | 恒常原則 A と実装・DoD のずれ | [L0](./L0-principles.md)、[DoD](./L2-components/dod.md) | 15m |
| 3 | Laws of UX アンカー差分 | [lawsofux.com](https://lawsofux.com/)、[ADR-004](./decisions/ADR-004-laws-of-ux.md) | 15m |
| 4 | 公開文書の機微混入チェック | [DOCUMENTATION](./DOCUMENTATION.md) / [MIRROR](./MIRROR.md) | 10m |
...
| [ADR-004](./decisions/ADR-004-laws-of-ux.md) 更新 PR | Laws of UX 側に追記・改名があり対応表へ影響するとき |
| [ROADMAP](./ROADMAP.md) 更新 | DS 技術投資・延期があるとき |
| GitHub issue | 実装 backlog（token / component / pattern）が出たとき |

原則を**変更しない**四半期でも記録は残す（「変更なし」と明記）。

---

### Runbook: 四半期 DADS 同期レビュー（#35）

[ADR-003](./decisions/ADR-003-dads-alignment.md) アンカーと DADS 側更新の差分棚卸し。初期スナップショットは [CHANGELOG](../CHANGELOG.md)。
...
| 影響 | 更新先 |
|------|--------|
| DADS 本体・tokens のバージョン変更 | [ADR-003](./decisions/ADR-003-dads-alignment.md) のアンカー表 + [CHANGELOG](../CHANGELOG.md) |
...
| 出力 | 条件 |
|------|------|
| 社内監査メモ + CHANGELOG 要約 | **毎回** — 5 面の差分一覧、ティア付け、判断 |
| ADR-003 / CHANGELOG PR | アンカーバージョンが変わったとき |
| #34 または新規 issue | 実装 backlog が出たとき |
| [ROADMAP](./ROADMAP.md) 更新 | 優先順位・フェーズ状態が変わったとき |

親トラッカー: #30 DADS Phase A〜E。
...
|------|------|
| Issue | #37 G5（前史 #3） |
| 代表 | アプリ型・LP 型の P1 各 1（具体名は社内） |
| 正本 | [`templates/consumer-g5/`](../templates/consumer-g5/) |
...
| **DS owner** | テンプレ・閾値・Runbook、DS 起因 regression の修正 |
...
4. 失敗時は DS（component / token）と product（ルーティング・データ・第三者 script）を切り分ける。
5. 週次 L3 で metrics とあわせ、consumer の例外・閾値緩和を issue に記録する。

初回 consumer green までのブロッカーは **consumer repo への PR**（本 DS リポジトリ外）。完了後 #37 に repo 名・workflow・代表 URL を記録する。

---

### i18n / 用語拡張（#38）

方針の正本は [ADR-005](./decisions/ADR-005-i18n-locale-architecture.md)。ここでは運用で参照する要約と、最初の locale 拡張チェックリストだけを置く。
...
3. DADS 名称変更は四半期 #35 の差分ルール経由で L4 対応表を更新する。
...
| # | Check |
|---|-------|
| 1 | L0 レビュー結果が CHANGELOG / ROADMAP（必要なら社内監査メモ）に残っている |
| 2 | DADS 同期結果が CHANGELOG / ROADMAP（必要なら社内監査メモ）に残っている |
| 3 | 変更があった場合、ADR / L0 / ROADMAP / CHANGELOG が同期されている |
| 4 | #17 / #35 に実施日とリンクをコメントした |
