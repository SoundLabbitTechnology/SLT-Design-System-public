...
[公開 L0](./L0-principles.md) と [Laws of UX](https://lawsofux.com/) アンカーの確認。事業投資・製品ロードマップの反証は社内方針側で行う。
...
前回レビュー日（または CHANGELOG 上の前回同期日）以降の更新のみを対象とする。
...
| CHANGELOG PR | アンカーバージョンが変わったとき |
| #34 または新規 issue | 実装 backlog・優先順位の変更 |
...
L5 の product 層品質を代表 consumer で継続検証する。実行は product repo。DS は ownership と triage の正本。

| 項目 | 内容 |
|------|------|
| Issue | #37 G5（前史 #3） |
| 代表 | アプリ型・LP 型の P1 各 1（具体名は社内） |
| 正本 | product repo の Lighthouse / E2E CI |
...
ここでは運用で参照する要約と、最初の locale 拡張チェックリストだけを置く（詳細方針は #38 / #39）。
...
| # | Check |
|---|-------|
| 1 | locale 境界（やること / やらないこと）を issue 本文に再掲している |
...
| # | Check |
|---|-------|
| 1 | L0 レビュー結果が CHANGELOG（必要なら社内監査メモ）に残っている |
| 2 | DADS 同期結果が CHANGELOG（必要なら社内監査メモ）に残っている |
| 3 | 変更があった場合、L0 / Issues / CHANGELOG が同期されている |
