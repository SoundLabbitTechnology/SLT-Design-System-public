...
| 項目 | 内容 |
|------|------|
| Visibility | **public（Docs site）** / 戦略は Private の internal（社内方針） |
| 文書番号 | SLT-DS-001-L0v1.0 |
| 版数 | v1.0（公開面: 恒常原則） |
| 更新日 | 2026-07-20 |
| 位置づけ | デザインシステム標準 §3。UI・体験の**品質判断**の優先順位 |
| 実装正本 | [`design-tokens/`](../design-tokens/) |
| 二リポ | 二リポ運用（Private 正本） — Private 実験 → Public 移行 |
...
認知・操作の共通語彙は [Laws of UX](https://lawsofux.com/) を参照モデルとする（方針: ADR-004（Private ADR））。**恒常原則 A を置き換えず補強する。** 構造・網羅・a11y の参照は ADR-003（DADS）（Private ADR）。
...
| Jakob's Law | A-1 | 慣習的 Header / Form 見た目（ADR-002（Private ADR）） |
...
3. **機械にも読める** — [`CLAUDE.md`](../CLAUDE.md) / [`llms.txt`](../llms.txt)（社内開発用）
