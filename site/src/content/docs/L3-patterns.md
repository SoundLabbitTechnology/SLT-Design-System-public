...
パターンの「なぜこの標準か」は [Laws of UX](https://lawsofux.com/) を UX 参照モデルとして明示する（ADR-004（Private ADR））。L0 恒常原則との接続は [L0 対応表](./L0-principles.md#ux-参照モデルlaws-of-ux)。DADS は構造・a11y（ADR-003（Private ADR））。
...
設計決定は ADR-002（Private ADR） を参照してください。
