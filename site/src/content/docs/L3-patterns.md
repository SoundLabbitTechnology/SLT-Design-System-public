...
2. Docs site `/components/` の標準 API を組み合わせる。
...
パターンの「なぜこの標準か」は [Laws of UX](https://lawsofux.com/) を UX 参照モデルとして明示する（[Laws of UX](https://lawsofux.com/)）。L0 恒常原則との接続は [L0 対応表](./L0-principles.md#ux-参照モデルlaws-of-ux)。DADS は構造・a11y（CHANGELOG）。
...
設計決定は Docs site `/components/site-header/` を参照してください。
