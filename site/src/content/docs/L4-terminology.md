...
**正本 locale は `ja`。** locale 拡張・翻訳 ownership は [L6](./L6-governance.md) と本表を正とする。`en` 列は推奨訳（UI / 文書で英語を出すときの揃え方）であり、定義の正本は常に `ja` です。
...
コンポーネント既定の accessible name / ボタン文言は 既定文言 inventory を参照してください。
...
6. **`en` 列** — `ja` 定義と同じ PR で推奨訳を入れる。訳だけ後追いで変えない。未翻訳は空欄より「TBD + owner」を残す。
7. **DADS 名称** — 四半期レビューで名称差分が出たら、本表の対応（推奨 / 避ける / `en`）を同サイクルで更新する。
