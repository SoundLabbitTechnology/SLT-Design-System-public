...
| 項目 | 内容 |
|------|------|
| Issue | [#37 G5](（Private issue）)（前史 #3） |
...
| L4 横断用語 | DS content | `ja` 定義正本。`en` 列は [L4 用語](./L4-terminology.md) に追加済（#39） |
...
| # | Check | 状態 |
|---|-------|------|
| 1 | ADR-005 の境界（やること / やらないこと）を issue 本文に再掲している | 済 |
| 2 | [L4-terminology.md](./L4-terminology.md) に `en` 推奨訳列（または同等の対応表）を追加する | 済 |
| 3 | 共有コンポーネントの既定文言 inventory（aria-label 等）を列挙する | 済 — [default-copy-inventory](./L2-components/default-copy-inventory.md) |
| 4 | override 不能な hardcoded 文言があれば props 化の PR 範囲を切っている | 済 — Dialog/Drawer/Toast/SiteHeader/FileUpload/PageNavigation/Skeleton/DataTable/Progress を props 化 |
| 5 | Docs / Storybook は翻訳しない、またはパイロット 1 ページまでに限定している | 済（翻訳しない） |
| 6 | product glossary との差分確認担当（DS content + product 1 名）を明記している | 済 — inventory に記載 |
| 7 | 完了後に ROADMAP の i18n 行を「`en` 用語列 + inventory 済」等へ更新する | 済 |
