...
8. `npm run check:public-export:check` で Public 向け機微スキャンを通す。
...
| 公開物 | 確認 |
|--------|------|
| Public Git tag | exports、型、CSS、CHANGELOG（`SLT-Design-System-public`） |
| | Docs site | Public の `site/` — canonical URL、内部リンク、ライブデモ |
| GitHub release | 変更要約、breaking / migration、既知の制約 |

## リリース後

- Public の Git tag / GitHub release / `package.json` version が一致していることを確認する。
- public-consumer-install.md（社内 adoption） に従い、代表 consumer（AI-Dash 等）を **Public tag** に bump する。
- 代表的な消費者で CSS import、theme attributes、主要 component を smoke test する。
- 問題があれば影響範囲、回避策、次の修正版を CHANGELOG と Issue に記録する。
- ロードマップ（Private `docs/ROADMAP.md`）の完了項目と残課題を更新する。
